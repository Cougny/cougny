...
export class Matchmaker {
   ...
   */
  enqueue(id: string, preferences: MatchPreferences = {}): Match | null {
    if (this.waiting.some((p) => p.id === id)) return null;

    const partnerIndex = this.findPartnerIndex(id, preferences);
    if (partnerIndex !== -1) {
      const [partner] = this.waiting.splice(partnerIndex, 1);
      return { a: partner!.id, b: id };
    }

    this.waiting.push({ id, preferences, enqueuedAt: Date.now() });
    return null;
  }
  ...
  /**
   * Pick the longest-waiting compatible peer. Compatibility today is: not self,
   * and — if both sides declared interests — at least one shared interest.
   * Locale/interest weighting can grow here without touching callers.
   */
  private findPartnerIndex(id: string, preferences: MatchPreferences): number {
